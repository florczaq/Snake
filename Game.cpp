...
void Game::render()
{
	...
	if (!paused && !end) window->draw(pauseButton);
...
void Game::update(sf::Clock& frameTime) {
	events(frameTime);
	if (frameTime.getElapsedTime().asMilliseconds() >= 70 * difficulty && !paused && !end)
		snakeMove(frameTime);
}
void Game::initVars(double width, double height) {
	...
	skinPickerButton = Button(pixelFont, width, height, "S", sf::Color(255,255,255,30), sf::Vector2f(width*0.85, height*0.05));
